...
drop database if exists DemoECommerce
go

create database DemoECommerce
go
...
(
	CartItemId ASC --ascending order
));
go
...
ALTER TABLE Orders  WITH CHECK ADD  CONSTRAINT FK_Orders_Carts FOREIGN KEY(CartId)
REFERENCES Carts (CartId)
GO

ALTER TABLE Orders CHECK CONSTRAINT FK_Orders_Carts
GO

ALTER TABLE Orders  WITH CHECK ADD  CONSTRAINT FK_Orders_Payments FOREIGN KEY(PaymentId)
...
ALTER TABLE Payments  WITH CHECK ADD  CONSTRAINT FK_Payments_PaymentMethods FOREIGN KEY(PaymentMethodId)
...
ALTER TABLE Payments  WITH CHECK ADD  CONSTRAINT FK_Payments_Users FOREIGN KEY(UserId)
REFERENCES Users (UserId)
GO

ALTER TABLE Payments CHECK CONSTRAINT FK_Payments_Users
GO

ALTER TABLE Products  WITH CHECK ADD  CONSTRAINT FK_Product_ProductCategories FOREIGN KEY(CategoryId)
REFERENCES ProductCategories (CategoryId)
GO

ALTER TABLE Products CHECK CONSTRAINT FK_Product_ProductCategories
GO
...
INSERT INTO ProductCategories (CategoryId, Category, SubCategory) VALUES (10, N'books', N'fiction')
INSERT INTO ProductCategories (CategoryId, Category, SubCategory) VALUES (11, N'books', N'non-fiction')
INSERT INTO ProductCategories (CategoryId, Category, SubCategory) VALUES (12, N'electronics', N'headphones')
INSERT INTO ProductCategories (CategoryId, Category, SubCategory) VALUES (13, N'electronics', N'speakers')
INSERT INTO ProductCategories (CategoryId, Category, SubCategory) VALUES (14, N'furniture', N'sofas')
INSERT INTO ProductCategories (CategoryId, Category, SubCategory) VALUES (15, N'furniture', N'dining tables')
INSERT INTO ProductCategories (CategoryId, Category, SubCategory) VALUES (16, N'clothes', N'women-dresses')
INSERT INTO ProductCategories (CategoryId, Category, SubCategory) VALUES (17, N'clothes', N'men-shirts')
INSERT INTO ProductCategories (CategoryId, Category, SubCategory) VALUES (18, N'clothes', N'men-pants')
...
INSERT PaymentMethods (PaymentMethodId, [Type], [Provider], Available, Reason) VALUES (1, N'Cash', N'', N'True', N'')
INSERT PaymentMethods (PaymentMethodId, [Type], [Provider], Available, Reason) VALUES (2, N'Card Payment', N'Bank', N'True', N'')
INSERT PaymentMethods (PaymentMethodId, [Type], [Provider], Available, Reason) VALUES (3, N'Net Banking', N'SBI', N'True', N'')
INSERT PaymentMethods (PaymentMethodId, [Type], [Provider], Available, Reason) VALUES (4, N'Net Banking', N'ICICI', N'True', N'')
INSERT PaymentMethods (PaymentMethodId, [Type], [Provider], Available, Reason) VALUES (5, N'Net Banking', N'Bank of Maharashtra', N'False', N'server down')
INSERT PaymentMethods (PaymentMethodId, [Type], [Provider], Available, Reason) VALUES (6, N'UPI', N'Google Pay', N'True', N'')
INSERT PaymentMethods (PaymentMethodId, [Type], [Provider], Available, Reason) VALUES (7, N'UPI', N'Phone Pay', N'True', N'')
INSERT PaymentMethods (PaymentMethodId, [Type], [Provider], Available, Reason) VALUES (8, N'UPI', N'BHIM Pay', N'True', N'')
...
INSERT INTO Users (UserId, FirstName, LastName, Email, [Address], Mobile, [Password], CreatedAt, ModifiedAt)
VALUES (1, N'John', N'Doe', N'john.doe@example.com', N'123 Main St, New York', N'123-456-7890', N'password123', GETDATE(), GETDATE()),
       (2, N'Jane', N'Smith', N'jane.smith@example.com', N'456 Oak St, Los Angeles', N'456-789-0123', N'password456', GETDATE(), GETDATE()),
       (3, N'Michael', N'Johnson', N'michael.johnson@example.com', N'789 Pine St, Chicago', N'789-012-3456', N'password789', GETDATE(), GETDATE()),
       (4, N'Sarah', N'Williams', N'sarah.williams@example.com', N'101 Elm St, Houston', N'101-112-1314', N'password1011', GETDATE(), GETDATE()),
       (5, N'David', N'Brown', N'david.brown@example.com', N'202 Cedar St, Philadelphia', N'202-213-1415', N'password1213', GETDATE(), GETDATE());
...
INSERT INTO Carts (CartId, UserId, Ordered, OrderedOn)
VALUES 
    (1,4, N'Yes', GETDATE()), -- Assuming UserId 1 placed an order
    (2,1, N'Yes', GETDATE()), -- Assuming UserId 3 placed an order
    (3,3, N'Yes', GETDATE()); -- Assuming UserId 5 placed an order
...
INSERT INTO CartItems (CartItemId, CartId, ProductId)
VALUES 
    (1,3,16), -- Assuming UserId 1 placed an order
    (2,1,7), -- Assuming UserId 3 placed an order
    (3,2,19); -- Assuming UserId 5 placed an order
...
VALUES
	(1, 1, 5000, 200, 0, 5200, '2024-02-07T10:00:00'),
	(2, 2, 7500, 300, 100, 7600, '2024-02-07T11:30:00'),
	(3, 1, 10000, 400, 200, 10200, '2024-02-07T12:45:00');
GO
...
VALUES 
    (5,1,3, GETDATE()), -- Assuming UserId 1 placed an order
    (2,3,1, GETDATE()), -- Assuming UserId 3 placed an order
    (4,2,2, GETDATE()); -- Assuming UserId 5 placed an order
...
SELECT Orders.Id, Users.FirstName, Users.LastName, Orders.CreatedAt
FROM Orders
JOIN Users ON Orders.UserId = Users.UserId;
GO

--Users and their total amount paid
SELECT UserId, SUM(AmountPaid) AS TotalAmountPaid
FROM Payments
GROUP BY UserId;
GO
...
--Retrieve users who have ordered but not paid
SELECT Users.UserId, Users.FirstName, Users.LastName
FROM Users
JOIN Orders ON Users.UserId = Orders.UserId
LEFT JOIN Payments ON Orders.Id = Payments.Id
WHERE Payments.Id IS NULL;
GO

--Complex queries with many-many as well
--Retrieve User Information with Payment Method Details for Orders Placed Today
SELECT u.FirstName, u.LastName, o.CreatedAt AS OrderPlacedAt, pm.[Type] AS PaymentMethodType, pm.Provider AS PaymentProvider
FROM Users u
...
SELECT u.UserId, u.FirstName, u.LastName, SUM(p.TotalAmount + p.ShippingCharges - p.AmountReduced) AS TotalSpent
FROM Users u
JOIN Orders o ON u.UserId = o.UserId
JOIN Payments p ON o.PaymentId = p.Id
...
--Retrieve Users and Their Associated Payment Method Type
SELECT u.UserId, u.FirstName, u.LastName, pm.[Type], pm.Type AS PaymentMethod
FROM Users u
JOIN Orders o ON u.UserId = o.UserId
JOIN Payments p ON o.PaymentId = p.Id
JOIN PaymentMethods pm ON p.PaymentMethodId = pm.PaymentMethodId
...
--Retrieve Payment Methods and Their Associated Users from Payments
SELECT pm.PaymentMethodId, pm.Type AS PaymentMethod, u.UserId, u.FirstName, u.LastName
